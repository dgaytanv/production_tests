...
#
# One replica = one linear chain: gsd_repN -> reco_repN -> nano_repN.
...
#                     [--eos-base /store/user/dgaytanv/production_tests/pre1_D121] \
#                     [--seed-base $((energy * 1000))] \
...

{
    ...
    for REP in $(seq 0 $((NREPLICAS - 1))); do
        SEED=$((SEED_BASE + REP))
        BASE_VARS="particle=\"${PARTICLE}\" energy=\"${ENERGY}\" seed=\"${SEED}\" replica=\"${REP}\" nevents=\"${NEVENTS}\" outdir=\"${OUTDIR}\" partname=\"${PARTNAME}\""
